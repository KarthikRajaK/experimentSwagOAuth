...
class Migration1536928368Inital extends MigrationStep
{
  ...
  `context_token` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`integration_id`) REFERENCES integration (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);'
        );
  ...
  PRIMARY KEY (`id`),
  FOREIGN KEY (`integration_id`) REFERENCES integration(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (`swag_oauth_refresh_token_id`) REFERENCES swag_oauth_refresh_token(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);'
        );
    }
